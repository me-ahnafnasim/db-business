...
| **`expo-image`** | Defer | Genuinely attractive — `ProductImage.js` is a single 61-line chokepoint for all product imagery (3 importers) currently using uncached RN core `Image`, so photos re-download on every grid scroll. But it is a dependency add, and this pass is UI-only. Recommend as a standalone follow-up. **Correction (2026-07-28):** this row previously said to pay for the size by running `npm run clean:icons` for "~2.7 MB". That saving did not exist. The script pointed at `node_modules/expo/node_modules/@expo/vector-icons/…`, which is not where the hoisted package lives, so it hit its `existsSync` guard and exited 0 every time — all 19 TTFs were still present after running it. It has been deleted. The real font saving was earned instead by consolidating source imports down to Feather + AntDesign, which Metro tree-shakes by reachability: 1,937,520 B → 186,080 B. |
